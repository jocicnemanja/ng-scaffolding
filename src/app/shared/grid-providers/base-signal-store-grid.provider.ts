...
export function withBaseTable<T extends { id: string }>() {
  return signalStoreFeature(
    ...
    withMethods((store) => {
      ...
      // Late-bound reference to the full store — set by _connectPipeline
      let _fullStore: any = null;

      // rxMethod: debounce → switchMap → fetchPage (provided by consumer)
      const _load = rxMethod<LoadParams>(
        pipe(
          debounceTime(0),
          switchMap((params) => {
            if (!_fullStore || typeof _fullStore.fetchPage !== 'function') return [];

            patchState(store, { loadingCounter: store.loadingCounter() + 1 });

            return _fullStore.fetchPage(params).pipe(
              tapResponse({
      ...
      return {
        ...
        // Stores reference to the full store and subscribes the Subject
        _connectPipeline(fullStore: any) {
    ...
    withHooks({
      onInit(store) {
        // Connect the Subject → rxMethod pipeline and trigger initial load
        (store as any)._connectPipeline(store);
...
export const BASE_GRID_PROVIDER_FACTORY = () => ({ provide: BASE_GRID_PROVIDER, useExisting: BASE_GRID_PROVIDER });
